...
def market_vectors(frame):
    ...
    arrays = [
        frame[c].to_numpy(dtype=float) / previous.to_numpy(dtype=float) - 1
    ...
    for name in ("oi_delta_pct", "funding_rate", "cvd"):
        ...
        mask = np.isfinite(v)
        arrays.append(np.where(mask, v, 0))
...
class RiskAwareTradingEnv(gym.Env):
    ...
    def __init__(self, frame, config, scaler, timeframe="1m", news_filter=None):
        ...
        self.duration = timeframe_to_timedelta(timeframe)
        self.symbol = config.get("data", {}).get("futures_symbol", "BTCUSDT")
        rl = config.get("rl", {})
        self.stop_pct = number(
            rl.get("stop_pct", 0.02), "RL stop_pct", maximum=1, positive=True
        )
        self.base_risk = number(
            rl.get("base_risk_percent", 0.02),
            "RL base_risk_percent",
            maximum=1,
            positive=True,
        )
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.i = 0
        self.done = False
        self.broker = PaperBroker(self.config, news_filter=deepcopy(self.news_filter))
        self.initial = self.broker.initial_balance
    ...
    def _obs(self):
        ...
        direction = (
            0 if pos is None else (1 if pos.direction == OrderDirection.LONG else -1)
        )
        market = (self.vectors[self.i] - self.scaler.mean) / self.scaler.scale
        values = np.r_[
            market,
            b.equity / self.initial,
            b.available_margin / self.initial,
            direction,
            b.circuit_breaker.risk_multiplier,
            self.drawdown,
        ]
    ...
    def step(self, action):
        ...
        b = self.broker
        before_realized = sum(t.net_pnl for t in b.trade_history)
        before_rejections = sum(
            o.status == OrderStatus.REJECTED for o in b.order_history
        )
        invalid = 0
        if not self.action_space.contains(action):
            ...
            direction = (
                OrderDirection.LONG if int(action) == 1 else OrderDirection.SHORT
            )
            price = float(self.frame.iloc[self.i].close)
            stop = price * (
                1 - self.stop_pct
                if direction == OrderDirection.LONG
                else 1 + self.stop_pct
            )
            b.submit_order(
                OrderRequest(
                    self.symbol,
                    ...
                    price,
                    stop,
                    self.frame.index[self.i] + self.duration,
            ...
            if self.symbol not in b.positions:
                invalid = 1
            else:
                b.request_close(self.symbol, self.frame.index[self.i] + self.duration)
        self.i += 1
        b.process_candle(self._candle(self.i))
        self.done = (
            self.i == len(self.frame) - 1 or b.is_halted or b.circuit_breaker.is_halted
        )
        if self.done:
            b.finalize(self.frame.index[self.i] + self.duration, force_close=True)
        rejected = (
            sum(o.status == OrderStatus.REJECTED for o in b.order_history)
            - before_rejections
        )
        ...
        reward = realized / self.initial - 0.5 * penalty - 1.0 * violation
        return (
            self._obs(),
            float(reward),
            bool(self.done),
...
def evaluate_policy(env, policy, seed=42):
    ...
    while not done:
        ...
        reward += r
        violations += info["risk_violation"]
    env.broker.verify_accounting_invariants()
    return {
        "reward": reward,
        "total_net_pnl": env.broker.equity - env.initial,
        "final_equity": env.broker.equity,
        "total_trades": len(env.broker.trade_history),
...
def train_ppo(
    ...
    if (output / "ppo_model.zip").exists() or (output / "research.sqlite").exists():
        raise ValueError("PPO output already exists; choose a new run directory")
    ...
    frozen = snapshot_run_config(config, news)
    env = RiskAwareTradingEnv(train, config, scaler, timeframe, news)
    ...
    validation_result = evaluate_policy(
        RiskAwareTradingEnv(validation, config, scaler, timeframe, news), policy, seed
    )
    test_env = lambda: RiskAwareTradingEnv(holdout, config, scaler, timeframe, news)
    result = evaluate_policy(test_env(), policy, seed)
    reloaded = evaluate_policy(test_env(), loaded_policy, seed)
    if result != reloaded:
        raise AssertionError("PPO save/load replay mismatch")
    ...
    def baseline(obs, env):
        ...
        if pos is None:
            return 1 if long else 2
        if (pos.direction == OrderDirection.LONG) != long:
            return 3
        return 0
    ...
    report = {
        ...
        "final_holdout": result,
        "no_trade": no_trade,
        "rule_based_ma_5_20": rule,
        "save_load_equal": True,
        "reward_units": "realized_USDT/initial_USDT - 0.5*incremental_fractional_DD - violations",
    }
    ...
    report["datasets"] = {
        k: dataset_manifest(
            f,
            config.get("research", {}).get("source", "SUPPLIED_DATA_NOT_VERIFIED"),
            "BTCUSDT",
            "perpetual",
            timeframe,
        )
    ...
    persist_research_run(output, "ppo", frozen, report)
    return report


def evaluate_saved_ppo(model_dir, frame, config, output_dir, timeframe="1m", seed=42):
    """Evaluate a persisted model/scaler without fitting or training anything."""
    import hashlib
    from stable_baselines3 import PPO
    ...
    payload = json.loads((directory / "ppo.report.json").read_text(encoding="utf-8"))
    manifest = json.loads((directory / "manifest.json").read_text(encoding="utf-8"))
    model_path = directory / "ppo_model.zip"
    model_digest = hashlib.sha256(model_path.read_bytes()).hexdigest()
    if model_digest != manifest["files"]["ppo_model.zip"]:
        raise ValueError("saved PPO model checksum mismatch")
    report_digest = hashlib.sha256(
        (directory / "ppo.report.json").read_bytes()
    ).hexdigest()
    if report_digest != manifest["files"]["ppo.report.json"]:
        raise ValueError("saved PPO metadata checksum mismatch")
    news = NewsCalendarFilter(config)
    frozen = snapshot_run_config(config, news)
    if compute_config_hash(frozen) != payload["config_hash"]:
    ...
    result = evaluate_policy(
        RiskAwareTradingEnv(frame, config, scaler, timeframe, news),
    ...
    report = {
        ...
        "training_code_commit_sha": payload["code_commit_sha"],
        "scaler_fit_scope": "loaded_train_only; no refit",
    ...
    persist_research_run(output_dir, "ppo_evaluation", frozen, report)
    return report

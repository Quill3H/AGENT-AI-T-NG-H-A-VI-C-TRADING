...
def frame_digest(frame):
    time_index(frame)
    return hashlib.sha256(frame.to_csv(index=True,float_format='%.17g',date_format='%Y-%m-%dT%H:%M:%S.%f%z').encode('utf-8')).hexdigest()
...
def dataset_manifest(frame,source,symbol,market_type,timeframe):
    ...
    return {'source':source,'symbol':symbol,'market_type':market_type,'timeframe':timeframe,
            'start':frame.index[0].isoformat() if len(frame) else None,
            'end':frame.index[-1].isoformat() if len(frame) else None,'rows':len(frame),
            'gaps':int((diffs>expected).sum()),'sha256':frame_digest(frame)}
...
def persist_research_run(output,name,config,report):
    ...
    payload=serializable({'name':name,'code_commit_sha':sha,'config':canonical,
                          'config_hash':compute_config_hash(canonical),'report':report})
    ...
    with sqlite3.connect(output/'research.sqlite') as db:
        db.execute('CREATE TABLE IF NOT EXISTS runs (name TEXT PRIMARY KEY, sha256 TEXT NOT NULL, payload TEXT NOT NULL)')
        prior=db.execute('SELECT sha256 FROM runs WHERE name=?',(name,)).fetchone()
        if prior and prior[0]!=digest:raise ValueError('research run identity conflict')
    ...
    _atomic_write_text(output/'config.canonical.json',json.dumps(canonical,sort_keys=True,indent=2,allow_nan=False))
    files={f'{name}.report.json':digest}
    if (output/'ppo_model.zip').is_file(): files['ppo_model.zip']=hashlib.sha256((output/'ppo_model.zip').read_bytes()).hexdigest()
    _atomic_write_text(output/'manifest.json',json.dumps({'name':name,'code_commit_sha':sha,
        'config_hash':payload['config_hash'],'files':files},indent=2,allow_nan=False))
    return output/f'{name}.report.json'


def persist_basket(output,result,config,manifest):
    report={'status':'AUTHOR_REPORTED / REVIEWER_NOT_VERIFIED','dataset':manifest,
            'metrics':result.metrics(),'basket':result.to_dict()}
    ...
    fig,ax=plt.subplots()
    if result.snapshots: ax.plot([s['timestamp'] for s in result.snapshots],[s['equity'] for s in result.snapshots])
    ax.set_ylabel('USDT');fig.autofmt_xdate();fig.savefig(output/'equity_curve.png');plt.close(fig)
    (output/'summary.md').write_text('Funding basket\n\nAUTHOR_REPORTED / REVIEWER_NOT_VERIFIED\n\n'

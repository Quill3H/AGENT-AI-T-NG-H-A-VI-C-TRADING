...
def number(value, name, minimum=0.0, maximum=None, positive=False):
    ...
    value = float(value)
    if not math.isfinite(value) or value < minimum or (positive and value == minimum) or (maximum is not None and value >= maximum):
...
def source_time(value, now, name, max_age_hours=24):
    if not isinstance(value, (pd.Timestamp, __import__('datetime').datetime)) or pd.isna(value):

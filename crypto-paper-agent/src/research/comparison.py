...
def walk_forward_splits(
    frame, train_bars, test_bars, step_bars=None, validation_bars=None
):
    time_index(frame)
    train_bars = positive_int(train_bars, "train_bars")
    test_bars = positive_int(test_bars, "test_bars")
    step = test_bars if step_bars is None else positive_int(step_bars, "step_bars")
    if step < test_bars:
        raise ValueError("overlapping OOS windows are prohibited")
    validation_bars = (
        max(1, train_bars // 5)
        if validation_bars is None
        else positive_int(validation_bars, "validation_bars")
    )
    if validation_bars >= train_bars:
        raise ValueError("validation must leave nonempty fit train")
    folds = []
    for start in range(0, len(frame) - train_bars - test_bars + 1, step):
        ...
        validation = frame.iloc[
            start + train_bars - validation_bars : start + train_bars
        ].copy()
    ...
    if not folds:
        raise ValueError("dataset is shorter than train_bars + test_bars")
    return folds
...
def compare_metrics(results):
    ...
    for name, metrics in results.items():
        if "total_net_pnl" not in metrics:
            raise KeyError(f"{name} is missing total_net_pnl")
        ...
        rows.append(
            dict(metrics, strategy=name, total_net_pnl=value, sample_count=count)
        )
    if not rows:
        return pd.DataFrame(columns=["strategy", "total_net_pnl", "sample_count"])
    return (
        pd.DataFrame(rows)
        .sort_values(["total_net_pnl", "strategy"], ascending=[False, True])
...
def run_walk_forward(
    ...
    folds = walk_forward_splits(frame, train_bars, test_bars, step_bars)
    rows = []
    aggregate = {}
    for fold in folds:
        for name, runner in strategy_runners.items():
            ...
            pnl = float(checked.total_net_pnl)
            count = int(checked.sample_count)
            rows.append(
                {
                    k: v
            ...
            agg["total_net_pnl"] += pnl
            agg["sample_count"] += count
            agg["fold_count"] += 1
    for a in aggregate.values():
        a["mean_fold_return_pct"] = (
            100 * a["total_net_pnl"] / (initial_capital * a["fold_count"])
        )

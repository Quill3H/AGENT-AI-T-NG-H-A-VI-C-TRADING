...
def _fetch_ohlcv_chunk(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            # params={"price": "mark"} không cần cho OHLCV; dùng default (index price)
...
def _candles_to_dataframe(candles: list) -> pd.DataFrame:
    ...
    # Thêm taker_buy_base_volume nếu có (Binance Futures trả về trong cột thứ 6)
    if candles and len(candles[0]) > 6:
        df["taker_buy_base_volume"] = [c[6] for c in candles]
    else:
        # Nếu API không trả về, ước tính 50% (không lý tưởng nhưng không crash)
